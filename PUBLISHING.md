# Publishing `pucora-websocket` as a standalone Go module

The implementation lives in `forks/pucora-websocket/` and is consumed by CE via a `replace` directive in the root `go.mod`:

```go
github.com/pucora/pucora-websocket/v2 => ./forks/pucora-websocket
```
...
3. Published dependencies already tagged on GitHub:
   ...
   - `github.com/pucora/pucora-jose/v2` (test dependency only — not required at runtime for the library)

The publish script creates `github.com/pucora/pucora-websocket` automatically when it is missing (requires `gh` and org access).
...
1. Copy `forks/pucora-websocket/` to a temporary directory
2. Strip `replace` directives from `go.mod` and run `go mod tidy`
3. Copy `LICENSE` from the CE root
4. Push to `git@github.com:pucora/pucora-websocket.git` on branch `main`
5. Create and push tag `v2.0.1`

## After publishing

Published module: https://github.com/pucora/pucora-websocket

1. Update the root `go.mod` require line when bumping versions:

   ```go
   github.com/pucora/pucora-websocket/v2 v2.0.1
   ```
...
`forks/pucora-websocket/.github/workflows/go.yml` is copied with the module when publishing. It runs `go test ./...` on every push/PR and on GitHub releases.
